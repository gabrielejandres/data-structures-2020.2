...
typedef struct team {
  ...
  node *last_index_queue; // aponta para o último elemento do time na fila
} Team;
...
void enqueue(queue *q, int value, int team_index, Team *teams) { 
  ...
  teams[team_index].last_index_queue = *tmp;
  // Coloca o novo elemento como o último
  updateLastElementOfTeamInQueue(team_index);
  ...
  while(t != 0) {
    ...
    while(strcmp(command,"STOP") != 0){
      if(strcmp(command,"ENQUEUE") == 0){
        ...
        if(team_index != -1) enqueue(teamQueue, element, team_index); // se o elemento não tiver time a função retorna -1
