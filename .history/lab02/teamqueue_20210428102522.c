...
void enqueue(queue *q, int value, int team_index, Team *teams) { 
  ...
  if (q->tail == NULL) {
      ...
      printf("%d", teams[team_index].last_element_queue);
      return;
  }

  printf("Elemento %d eh o ultimo do time %d na fila", teams[team_index].last_element_queue->key, teams[team_index].last_element_queue->team_index);
  // Se houver alguém do time do novo elemento na fila, ele deve entrar logo atrás
  if(teams[team_index].last_element_queue->team_index == team_index) {
    printf("Os dois %d e %d sao do time %d \n", teams[team_index].last_element_queue, tmp->key, team_index);
    ...
    teams[team_index].last_element_queue = tmp;
      ...
      printf("Os dois %d e %d nao sao do mesmo time \n", teams[team_index].last_element_queue, tmp->key);
