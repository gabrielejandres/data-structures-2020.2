...
struct queue* newNode(int value, int team_index)
{
    struct queue* tmp = (node*) malloc(sizeof(node));
...
void enqueue(struct queue *q, int value, int team_index) {
  // Cria um novo nó da lista encadeada
  struct queue *tmp = newNode(value, team_index);
