...
void enqueue(queue *q, int value, int team_index, Team *teams[]) { 
  ...
  if (q->tail == NULL) {
      ...
      printf("Ultimo elemento da fila desse time %d", teams[team_index]->last_element_queue->key);
      return;
  }
...
int main(int argc, char *argv[ ]){
  ...
  while(t != 0) {
    ...
    teams[t] = (Team*) malloc(sizeof(Team) * numberOfTeams);
    ...
    while(strcmp(command,"STOP") != 0){
      if(strcmp(command,"ENQUEUE") == 0){
        int element;
        scanf("%d",&element);
        int team_index = searchTeamOfAnElement(element, pointer_teams, t);
        if(team_index != -1) enqueue(teamQueue, element, team_index, pointer_teams); // se o elemento não tiver time a função retorna -1
        ...
        int id = dequeue(teamQueue, pointer_teams);
        printf("%d\n", id);
        // printf("Cabeca: %d \n", *(teamQueue->head));
        // printf("Cauda: %d \n", *(teamQueue->tail));
      }
